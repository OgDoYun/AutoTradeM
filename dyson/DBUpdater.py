...
# How to start mariadbd deamon(mysql server) for Mac.
# mysql.server start
...
class DBUpdater:
    ...
    def read_krx_code(self):
        """KRX로부터 상장법인목록 파일을 만들어서 데이터 프레임으로 반환"""
    ...
    def read_naver(self, code, company, pages_to_fetch):
        ...
        try:
            ...
            for page in range(1, pages + 1):
                ...
                req_pg_url = requests.get(pg_url, headers={'User-Agent' : 'Mozilla/5.0'}).text
                df = df.append(pd.read_html(req_pg_url, header=0)[0])
                tmnow = datetime.now().strftime('%Y-%m-%d %H:%M')
